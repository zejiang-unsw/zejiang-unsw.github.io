...
```{r wavelet-transforms, message=TRUE}
# data generation
...
```{r tab1, echo=FALSE, results='hold'}
tab1 <- data.frame(
...
```{r vt, warning=FALSE, results = "hide", message=FALSE}
if (FALSE) {
  ### Synthetic example
  # data generation
  set.seed(2020)
...
```{r variance-transform, message=FALSE, results = "hide", warning=FALSE, fig.keep='last', fig.cap='Orignal and VT predictors. (a): DWT-MRA (b): MODWT/AT', fig.width=9, fig.height=6}
#-------------------------------------------------------------------
if (FALSE) {
  set.seed(2020)
